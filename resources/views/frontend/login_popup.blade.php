      ...
      <div class="modal-body p-4">
        ...
        <form method="POST" action="{{ route('user.login_submit') }}">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control form-control-lg" id="mobile" name="mobile" placeholder="Enter Mobile Number" value="{{ old('mobile', session('mobile')) }}" required pattern="\d{10}" maxlength="10">
          </div>

          <!-- OTP input shown only if session has showOtpField -->
          @if(session('showOtpField'))
          <div class="mb-3">
            <label for="otp" class="form-label fw-semibold">Enter OTP sent to +91 {{ session('mobile') }}</label>
          ...
          <button type="submit" class="btn btn-primary btn-lg w-100 fw-semibold mb-3">
            @if(session('showOtpField')) Verify OTP & Login @else Send OTP @endif
          </button>
...
<div class="text-center mt-3">
  <h6>If you're a new user, please <a href="{{ route('user.signup') }}" class="text-primary fw-bold">click here to register</a>.</h6>
</div>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById('displayMobile').textContent = "{{ session('mobile') }}";
});
</script>
@endif
