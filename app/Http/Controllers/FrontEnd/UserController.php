...
public function login(Request $request)
{
    ...
    $information = [
        'seoInfo' => $language->seoInfo()->select('meta_keyword_login', 'meta_description_login')->first(),
        'pageHeading' => $misc->getPageHeading($language),
        'bgImg' => $misc->getBreadcrumb(),
        'bs' => Basic::query()->select('google_recaptcha_status', 'facebook_login_status', 'google_login_status', 'whatsapp_login_status')->first(),
        'digitalProductStatus' => $request->input('digital_item') ?? null
    ];
    ...
    // Pass mobile number from session if available
    $information['mobile'] = Session::get('mobile');
...
public function sendOTP(Request $request)
{
    ...
    try {
        ...
        // Send via WhatsApp
        $response = $this->sendWhatsAppOTP($mobile, $otp);
...
public function verifyOTP(Request $request)
{
    ...
    try {
        ...
        $user = User::firstOrCreate(
            ['mobile' => $request->mobile],
            ['status' => 1, 'email_verified_at' => now()]
        );
        ...
        return response()->json([
            ...
            'redirect' => $request->session()->get('redirectTo', route('user.dashboard'))
        ]);
...
{
    $url = 'https://whatsapp.databridgecrm.com/api/create-message';
    $data = [
        'appkey' => 'd20b3a20-3fe5-41a1-aeaf-13373dc30936',
        'authkey' => '7M9Bx35i1f6TxMVDE3M3UQOvWhX1G3e16exDeBmEzqFTU3wcDz',
        'to' => '91' . $mobile,
        'message' => "💥 Welcome to *Khiladi India*! 💥\n\n🔑 Your One Time Password (OTP) is: *$otp*\n\n⏳ This OTP is valid for the next 5 minutes.\n\n⚠️ Do not share this OTP with anyone for your account safety.\n\nThank you,\nTeam Khiladi India 🇮🇳"
    ];
    ...
    'success' => isset($result['message_status']) && strtolower($result['message_status']) === 'success',
    'message' => $result['message_status'] ?? 'Unknown response'
];
...
//   isme login Submit functiion me change kiya hai pure me 

public function loginSubmit(Request $request)
{
    // OTP request phase (first step)
    if (!$request->has('otp')) {
        $request->validate([
        ...
        // If not registered → create a placeholder user (with only mobile for now)
        if (!$user) {
            $user = new User();
        ...
        $whatsappResponse = $this->sendWhatsAppOTP($user->mobile, $otp);

        if (!$whatsappResponse['success']) {
            \Log::error("WhatsApp OTP failed for {$user->mobile}: " . $whatsappResponse['message']);
        ...
        return back()->with([
            'showOtpField' => true,
            'mobile' => $request->mobile,
            'isNewUser' => empty($user->name), // if user has no name, it’s signup
            'success' => 'OTP sent successfully on WhatsApp!'
        ]);
    }
    ...
    // If user was newly created and has no name yet → save name
    if (empty($user->name) && $request->filled('name')) {
...
//   isme signup controller me change hua hai mobile number add kiya hai 
...
public function signupSubmit(Request $request)
{
    ...
    $user->mobile = $request->mobile;
    $user->status = 1;
    // $user->password = Hash::make($request->password); // password removed
