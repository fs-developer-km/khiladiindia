...
public function sendOtp(Request $request)
{
    ...
    if ($validator->fails()) {
        Log::warning("Vendor signup validation failed", $validator->errors()->toArray());
    ...
    // Save OTP in session
    session([
        'signup_data' => $request->only(['username', 'email', 'phone']),
        'otp' => $otp,
        'otp_expires_at' => $expiresAt,
    ]);

    Log::info("Generated OTP for signup", [
        'phone' => $request->phone,
        'otp' => $otp,
    ...
    $response = $this->sendWhatsAppOTP($request->phone, $otp);

    if (!$response['success']) {
        Log::error("WhatsApp OTP failed", $response);
        return response()->json(['success' => false, 'message' => $response['message']]);
    }

    Log::info("WhatsApp OTP sent successfully", $response);

    return response()->json(['success' => true, 'message' => 'OTP sent to WhatsApp']);
}

/**
 * WhatsApp OTP Sender using cURL
 */
private function sendWhatsAppOTP($mobile, $otp)
{
    ...
    Log::info("Sending WhatsApp OTP request", ['url' => $url, 'payload' => $data]);
    ...
    if ($error) {
        Log::error("cURL error while sending WhatsApp OTP", ['error' => $error]);
        return ['success' => false, 'message' => $error];
    }

    $result = json_decode($response, true);

    Log::info("WhatsApp OTP API response", ['response' => $response, 'decoded' => $result]);
...
public function create(Request $request)
{
    ...
    if (!$sessionOtp || !$signupData) {
        return back()->with('error', 'Session expired. Please try again.');
    }

    if ($sessionOtp != $request->otp || now()->gt($expiresAt)) {
        return back()->with('error', 'Invalid or expired OTP');
    }

    // All good → Create vendor
    $in = $signupData;
    $in['status'] = 1; // direct active after OTP
    $in['email_verified_at'] = now(); // ✅ Mark email verified
    ...
    // Clear session OTP
    session()->forget(['signup_data', 'otp', 'otp_expires_at']);

    Auth::guard('vendor')->login($vendor);

    return redirect()->route('vendor.dashboard')->with('success', 'Signup successful!');
}
...
{
    $request->validate([
        'phone' => 'required|digits:10|exists:vendors,phone'
    ]);
...
public function loginWithOtp(Request $request)
{
    ...
    // Clear session
    session()->forget(['login_phone', 'login_otp', 'login_otp_expires_at']);
